...
const CompanyProfile = () => {
  ...
  const { contactRoles, playerPositions } = useAppContext();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Emit event to update AppLayout with new company data
      window.dispatchEvent(new CustomEvent('companyProfileUpdated', {
        detail: { companyName, logoUrl }
      }));
    } catch (err) {
      console.error(err);
    } finally {
  ...
  //           <p className="text-white font-semibold">Saving...</p>
  //         </div>
  //       </div>
  //     )}
  //     <Card>
      ...
      {saving && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="flex flex-col items-center gap-4 bg-white rounded-xl p-8 shadow-2xl">
            ...
            <p className="text-lg font-semibold text-gray-800">Saving Changes...</p>
